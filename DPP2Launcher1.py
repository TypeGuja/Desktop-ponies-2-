...
def get_resource_path(relative_path):
    """Получает правильный путь при упаковке в EXE или при обычном запуске"""
    try:
        # PyInstaller создает временную папку и сохраняет путь в _MEIPASS
        base_path = sys._MEIPASS
    except Exception:
        # Если не в PyInstaller, используем текущую директорию
...
def get_original_path(relative_path):
    """Получает путь к файлу в оригинальной папке с EXE"""
    # Определяем папку, где находится EXE файл
...
# ПУТИ для реального запуска (рядом с EXE файлом)
CLIENT_FILE = get_original_path(r"DPP2serverUDP\Client\main.py")
CLIENT_OFFLINE_FILE = get_original_path(r"DPP2.py")
SERVER_FILE = get_original_path(r"DPP2serverUDP\Server\main.py")
...
class UltraModernLauncher:
    def __init__(self):
        ...
        self.current_process = None  # Текущий запущенный процесс
    ...
    def check_files(self):
        ...
        files = [
            ("КЛИЕНТ", CLIENT_FILE),
            ("ОФЛАЙН КЛИЕНТ", CLIENT_OFFLINE_FILE),
            ("СЕРВЕР", SERVER_FILE)
        ]
        ...
        for name, path in files:
            if os.path.exists(path):
                ...
                print(f"✗ {name}: {os.path.abspath(path)} - НЕ НАЙДЕН!")
    ...
    def run_python_script_and_wait(self, script_path, script_name):
        """Запуск Python скрипта и НЕ ждём (открываем и забываем)"""
        try:
            # Проверяем существует ли файл
            if not os.path.exists(script_path):
                # Пытаемся найти файл рядом с EXE
                exe_dir = os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else os.getcwd()
                script_name_only = os.path.basename(script_path)
                alt_path = os.path.join(exe_dir, script_name_only)
            ...
            if not work_dir:  # Если нет директории (файл в текущей папке)
                work_dir = os.getcwd()

            print(f"\n🚀 Запуск {script_name}:")
            print(f"📁 Файл: {os.path.abspath(script_path)}")
            print(f"📂 Директория: {work_dir}")

            # Для Windows - запускаем БЕЗ ожидания
            if os.name == 'nt':
                # КОМАНДА которая открывает и не ждёт
                cmd = f'start "" "{sys.executable}" "{script_path}"'
            ...
            # НЕ показываем лаунчер сразу, оставляем его скрытым
            # Или показываем через 1 секунду
    ...
    def launch_client(self):
        ...
        def launch():
            success = self.run_python_script_and_wait(CLIENT_FILE, "Client")
            if not success:
    ...
    def launch_client_offline(self):
        ...
        def launch():
            success = self.run_python_script_and_wait(CLIENT_OFFLINE_FILE, "Client Offline")
            if not success:
                self.root.after(0, self.show_window)
    ...
    def launch_server(self):
        ...
        def launch():
            success = self.run_python_script_and_wait(SERVER_FILE, "Server")
            if not success:
                self.root.after(0, self.show_window)
    ...
    def launch_all(self):
        ...
        def launch():
            ...
            server_success = self.run_python_script_and_wait(SERVER_FILE, "Server")
            if server_success:
                # Сервер завершился, можно показать лаунчер
                self.root.after(0, self.show_window)
            else:
                self.root.after(0, self.show_window)
    ...
    def quit_launcher(self):
        """Выход из лаунчера"""
        # Если есть запущенный процесс, завершаем его
...
if __name__ == "__main__":
    ...
    print(f"CLIENT_FILE: {os.path.abspath(CLIENT_FILE)}")
    print(f"CLIENT_OFFLINE_FILE: {os.path.abspath(CLIENT_OFFLINE_FILE)}")
    print(f"SERVER_FILE: {os.path.abspath(SERVER_FILE)}")
    print("=" * 50)
