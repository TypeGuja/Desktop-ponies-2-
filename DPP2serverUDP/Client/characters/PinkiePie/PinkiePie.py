...
class GIFPlayer:
    def __init__(self, root, initial_scale=1.0):
        ...
        self.GIF_FOLDER = os.path.join(current_dir, "Pinkie Pie_Gifct")

        # ========== ПУТИ ДЛЯ ГИФОК ==========
        self.GIF_PATHS = {
            # Stand гифки
            "stand_right": "stand_pp_right.gif",
            "stand_left": "stand_pp_left.gif",

            # Movement гифки
            "move_right": "trotcycle_pp_right.gif",
            "move_left": "trotcycle_pp_left.gif",

            # Sleep гифки (в подпапке sleep-pp)
            "sleep_right": os.path.join("sleep-pp", "pp-sleep-right.gif"),
            "sleep_left": os.path.join("sleep-pp", "pp-sleep-left.gif"),

            # Drag гифки (в подпапке drag-pp)
            "drag": os.path.join("drag-pp", "drag_pp_left.gif")
        }
        ...
        self.base_sleep_width = 90
        self.base_sleep_height = 70
        ...
        self.SLEEP_FRAME_DURATION_MS = 100
    ...
    def change_scale(self, new_scale):
        ...
        try:
            ...
            print(f"📏 Изменение масштаба Pinkie Pie на {scale_percent}%")
    ...
    def _is_in_push_zone(self, x, y):
        ...
        try:
            ...
            # Проверяем нижнюю зону выталкивания
    ...
    def _safe_move_to_target(self):
        ...
        while (self.moving and not self.is_dragging and
            ...
            try:
                ...
                # Проверка на нахождение в зоне выталкивания во время движения
                if self._is_in_push_zone(current_x, current_y):
                    new_x, new_y = self._apply_push_force(current_x, current_y)
    ...
    def _do_drag(self, event):
        ...
        if (self.is_dragging and not self._shutdown_flag.is_set() and
            ...
            # Проверка и выталкивание при перетаскивании в зону выталкивания
    ...
    def _end_drag(self, event):
        ...
        # Проверка и выталкивание после окончания перетаскивания
        if self._is_in_push_zone(current_x, current_y):
            new_x, new_y = self._apply_push_force(current_x, current_y)
            self.root.geometry(f"+{new_x}+{new_y}")
    ...
    def _check_gif_folder(self):
        ...
        if not os.path.exists(self.GIF_FOLDER):
            ...
            try:
                ...
                print("✅ Папка создана")
                sleep_folder = os.path.join(self.GIF_FOLDER, "sleep-pp")
                drag_folder = os.path.join(self.GIF_FOLDER, "drag-pp")
    ...
    def _exit_program(self):
        ...
        print("🛑 Завершение программы через Pinkie Pie...")
        self._stop_all_threads()
        self._clear_canvas_completely()
        import sys
        sys.exit(0)
    ...
    def _create_fallback_animation(self):
        ...
        try:
            ...
            for i, color in enumerate(colors):
                ...
                draw.text((center_x - 10, center_y - 5), "PP", fill="white")
    ...
    def _stop_all_threads(self):
        ...
        print("🛑 Мягкая остановка потоков Pinkie Pie...")
