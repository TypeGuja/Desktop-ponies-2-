...
class GIFPlayer:
    def __init__(self, root, initial_scale=1.0):
        ...
        self.GIF_FOLDER = os.path.join(current_dir, "Fluttershy_Gifct")

        # ========== ПУТИ ДЛЯ ГИФОК ==========
        self.GIF_PATHS = {
            # Stand гифки
            "stand_right": "stand_fs_right.gif",
            "stand_left": "stand_fs_left.gif",

            # Movement гифки
            "move_right": "trotcycle_fs_right.gif",
            "move_left": "trotcycle_fs_left.gif",

            # Sleep гифки (в подпапке sleep-fs)
            "sleep_right": os.path.join("sleep-fs", "sleeping_fs_right.gif"),
            "sleep_left": os.path.join("sleep-fs", "sleeping_fs_left.gif"),

            # Drag гифки (в подпапке drag-fs)
            "drag": os.path.join("drag-fs", "fs_drag_left.gif")
        }
    ...
    def change_scale(self, new_scale):
        ...
        try:
            ...
            print(f"📏 Изменение масштаба Fluttershy на {scale_percent}%")
    ...
    def _check_gif_folder(self):
        ...
        if not os.path.exists(self.GIF_FOLDER):
            ...
            try:
                ...
                print("✅ Папка создана")
                sleep_folder = os.path.join(self.GIF_FOLDER, "sleep-fs")
                drag_folder = os.path.join(self.GIF_FOLDER, "drag-fs")
    ...
    def _exit_program(self):
        ...
        print("🛑 Завершение программы через Fluttershy...")
        self._stop_all_threads()
        self._clear_canvas_completely()
        import sys
        sys.exit(0)
    ...
    def _create_fallback_animation(self):
        ...
        try:
            ...
            for i, color in enumerate(colors):
                ...
                draw.text((center_x - 10, center_y - 5), "FS", fill="white")
    ...
    def _stop_all_threads(self):
        ...
        print("🛑 Мягкая остановка потоков Fluttershy...")
