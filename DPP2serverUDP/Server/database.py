...
class Database:
    def __init__(self, db_path='game_server_db.json'):
        ...
        self.data = {
            ...
            'world_data': {
                ...
                'day': 1,
                'online_players': 0,
                'protocol': 'udp',  # Добавляем протокол
                'udp_port': 5555  # Добавляем UDP порт
            },
            'gifct_settings': {
                'gifct_enabled': {
                    'Gifct1': True,
    ...
    def create_character(self, player_id, character_data):
        ...
        character = {
            ...
            'gifct': character_data.get('gifct', {
                'Gifct1': character_data.get('gifct1', self.data['gifct_settings']['gifct_configs']['Gifct1']),
                'Gifct2': character_data.get('gifct2', self.data['gifct_settings']['gifct_configs']['Gifct2'])
            }),
            'creation_time': datetime.now().isoformat(),
            'last_played': datetime.now().isoformat(),
            'last_activity': datetime.now().isoformat(),  # Для UDP
            'playtime': 0,
            'achievements': [],
            'quests': {
                'active': [],
    ...
    def update_character(self, character_id, updates):
        ...
        if character_id in self.data['characters']:
            ...
            # Для UDP обновляем время последней активности
            if 'position' in updates or 'last_activity' not in updates:
                self.data['characters'][character_id]['last_activity'] = datetime.now().isoformat()
